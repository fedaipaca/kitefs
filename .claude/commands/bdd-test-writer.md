...
Write the BDD tests for a single BDD task. This prompt consumes a finished BDD task spec, such as the output of the `/bdd-task` command, and turns it into runnable `pytest` + `pytest-bdd` coverage.

Assume the general repository guardrails in the root `CLAUDE.md` are already active. This prompt adds only task-specific rules for authoring BDD tests.

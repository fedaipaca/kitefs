...
**Goal:** Create a pip-installable package with the `src/kitefs/` building-block layout and dev tooling for local-only development.
**Description:** Set up the `src/kitefs/` source tree with one sub-package per building block (sdk, cli, definitions, registry, validation, offline_store, online_store, join_engine, providers/base + providers/local, config, errors). Configure `pyproject.toml` (uv) with base dependencies only — no AWS dependencies at this stage. Set up `justfile`, ruff config, pytest config, and the `kitefs` console-script entry point. Confirm `pip install -e .` succeeds and `kitefs --help` prints a placeholder. AWS packaging is introduced later in P-12.
**Requirements and References:** [CON-001](02-product-requirements.md#con-001--python-312), [CON-002](02-product-requirements.md#con-002--pip-installable-library), [CON-005](02-product-requirements.md#con-005--no-server-or-daemon), [NFR-MAINT-001](02-product-requirements.md#nfr-maint-001--modular-architecture), [Packaging Model](04-architecture.md#packaging-model)

### T-002 — Error Model and Shared Enums

**Status:** not started
**Branch:** `feat/T-002-errors-and-enums`
**Goal:** Establish the shared exception hierarchy (BB-11) and the shared enums used across definitions, configuration, and validation.
**Description:** Create `src/kitefs/errors/` with the base `KiteFSError` and all public exception classes per the contracts doc, with actionable error message formatting helpers. Create the `FeatureType`, `StorageTarget`, and `ValidationMode` enums. Re-export the public exception classes and enums from the top-level package. No behavior logic.
**Requirements and References:** [AP-7](04-architecture.md#architectural-design-principles), [Exception Hierarchy](06-api-and-cli-contracts.md#exception-hierarchy), [FR-DEF-002](02-product-requirements.md#fr-def-002--field-type-definitions), [FR-DEF-003](02-product-requirements.md#fr-def-003--storage-target), [FR-DEF-005](02-product-requirements.md#fr-def-005--per-operation-validation-modes)
